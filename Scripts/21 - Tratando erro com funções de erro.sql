...
BEGIN
	BEGIN TRY 
		...
		SELECT SUM(A.QUANTIDADE * A.PREÇO) AS FATURAMENTO, SUM(A.QUANTIDADE * A.PREÇO)/@DENOMINADOR AS COMISSAO
...
EXEC TrataErroZeroTry2 @CPF = @CPF, @DENOMINADOR = @DENOMINADOR, @ANO = @ANO, @MENSAGEM = @MENSAGEM OUTPUT
